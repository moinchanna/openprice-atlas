...
## Accessible VoiceBox Theme Extension

### Colors & Palette Mapping

| Context | Light Theme | Dark Theme | Role |
| :--- | :--- | :--- | :--- |
| **Page Background** | `#FAFAFA` | `#0A0A0A` | Main document canvas |
| **Main Surface** | `#F5F5F5` | `#121212` | Default component container background |
| **Secondary Surface** | `#E5E5E5` | `#1A1A1A` | Raised or active elements |
| **Hover Surface** | — | `#262626` | List and card cursor hover highlights |
| **Primary Text** | `#0A0A0A` | `#FAFAFA` | Main headings, table fields, copy |
| **Secondary Text** | `#404040` | `#D4D4D4` | Form labels, description notes, bylines |
| **Muted Text** | `#666666` | `#A3A3A3` | Secondary helper descriptions and trust strips |
| **Medium Border** | `#BDBDBD` | `#525252` | Grid dividers, input outlines, inactive cards |
| **Strong Border** | `#0A0A0A` | `#FAFAFA` | Primary active borders and title dividers |
| **Accent Red** | `#EF4444` | `#EF4444` | Contrast indicators, hover colors, active state underline |

### Accessibility Sizing Guidelines

- **Root Body Default**: `18px`
- **Descriptions & Prose**: `18px` (`line-height: 1.55 - 1.7`, `max-width: 65-75 chars`)
- **Helper text & Trust indicators**: `15px`
- **Form labels & Table headings**: `14px`
- **Interactive Buttons**: `16px` (`font-weight: 700`, `letter-spacing: 0.05em`)
- **Headers & Navigation Links**: `15px`
- **Inputs & Select fields**: `17px` (`min-height: 50px`, `border: 2px solid`)
- **Table cell body text**: `16px` - `17px` (`row-height: 68px` minimum)
- **Summary blocks**: Label `14px` / Pricing value `30px` minimum

### Theme Lifecycle Lifecycle
1. Blocking client-side script initializes Class selector on `html` tag.
2. Synchronizes preferences in `localStorage` under `openprice_atlas_theme`.
3. Auto-detects OS `prefers-color-scheme: dark` settings on clean initialization.
